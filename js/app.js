...
function createBookCard(meta, isOffline = false, isVerified = false) {
    ...
    // Direct Download & Seal Logic via Serverless Proxy
    ...
    if (dlBtn) {
        dlBtn.addEventListener('click', async (e) => {
            ...
            try {
                ...
                // Save both the metadata and the raw Binary Buffer 
                await window.dbAPI.saveBook(bookId, arrayBuffer, meta);

                // Auto-Open the sealed Offline copy
                openBook(bookId);

            } catch (err) {
                console.error("Mühürleme hatası (Sealing Error):", err);
                dlBtn.textContent = 'Hata! Tekrar Dene';
                dlBtn.disabled = false;
                dlBtn.style.background = 'var(--error)';
            }
        });
    }
...
async function setupNetworkReader(meta) {
    // V10: All network reading now routes through proxy and seals to DB immediately. No more live fetches.
    const bookId = `off_${meta.id || Date.now()}`;

    // Check if already downloaded
    const existing = await window.dbAPI.getBook(bookId);
    ...
    try {
        ...
        console.error("Kitap açılırken ağ hatası:", err);
        alert("Kitap indirilemedi. Lütfen bağlantınızı kontrol edin.");
    }
}
